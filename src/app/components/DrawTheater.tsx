...
const SEG_COLORS = [
  '#0d5e1e','#1a7a2e','#0f9e36','#1BF26A','#13c455','#0aad48',
  '#44FF44','#2ee870','#0d7a30','#26d95f','#0c6628','#3fff88',
  '#0e8c35','#15b84d','#06522a','#52f098',
];
const N_SEGS = 16;
  ...
  return (
    <div style={{ position:'relative', display:'inline-block' }}>
      ...
      <svg width="240" height="240" style={{
        ...
        filter: spinning ? 'drop-shadow(0 0 28px rgba(27,242,106,.9))' : 'drop-shadow(0 0 14px rgba(27,242,106,.4))',
        display:'block',
      }}>
        ...
        {Array.from({length:N_SEGS},(_,i) => {
          ...
          const fill = wonRank>=0 ? ['#FFD700','#C0C0C0','#CD7F32'][wonRank] : SEG_COLORS[i%SEG_COLORS.length];
          const pt = tp(i);
          const rot = ((i+0.5)/N_SEGS*360-90) + (pt.x<cx?180:0);
          const light = ['#1BF26A','#44FF44','#2ee870','#3fff88','#52f098','#26d95f'].includes(fill);
          return (
            <g key={i}>
              <path d={seg(i)} fill={fill} stroke={wonRank>=0?'#fff':'#0a2e10'} strokeWidth={wonRank>=0?2.5:1.5}/>
              <text x={pt.x} y={pt.y} textAnchor="middle" dominantBaseline="middle"
                fill={light?'#0a2e10':'#fff'} fontSize={6} fontFamily="'Press Start 2P',monospace"
        ...
        {Array.from({length:N_SEGS},(_,i) => {
          ...
          return <line key={i} x1={cx+(r+2)*Math.cos(a)} y1={cy+(r+2)*Math.sin(a)} x2={cx+(r+9)*Math.cos(a)} y2={cy+(r+9)*Math.sin(a)} stroke="#1BF26A" strokeWidth="2"/>;
        })}
...
export default function DrawTheater({ onClose, onPointsEarned, activePerks }) {
  ...
  // ── runDrawRef: always points to latest runDraw ─────────────
  const runDrawRef = useRef(null);
  const angleRef   = useRef(0);
  const phaseRef   = useRef('waiting');

  // ── runDraw — defined FIRST so ref can be set below ─────────
  ...
  // ── Keep ref in sync — AFTER runDraw is declared ────────────
  ...
  useEffect(() => {
    ...
    const tick = setInterval(() => {
      ...
      if (rem <= 0) {
        clearInterval(tick);
        // Small delay then fire via ref — guaranteed fresh
        setTimeout(() => {
    ...
    <div style={{
      ...
      {confetti&&(
        <div style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',zIndex:2001,pointerEvents:'none'}}>
          <div style={{background:'rgba(10,30,10,.92)',border:'3px solid #FFD700',boxShadow:'0 0 60px rgba(255,215,0,.6)',padding:'32px 48px',textAlign:'center'}}>
      ...
      {/* Header */}
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',background:'rgba(10,50,10,.98)',borderBottom:'2px solid #1BF26A',flexShrink:0,flexWrap:'wrap',gap:8}}>
        <span style={{color:'#FFDD00',fontSize:12,letterSpacing:2}}>🎲 DRAW ROOM</span>
        <select value={selectedPool.id} onChange={e=>setSelectedPool(POOLS.find(p=>p.id===e.target.value)||POOLS[0])}
          style={{background:'#145414',color:'#1BF26A',border:'2px solid #1BF26A',padding:'8px 12px',fontSize:10,fontFamily:"'Press Start 2P',monospace",cursor:'pointer',outline:'none'}}>
          {POOLS.map(p=><option key={p.id} value={p.id}>{p.icon} {p.name} — {p.label}</option>)}
        </select>
        <button onClick={onClose} style={{background:'#2a0808',border:'2px solid #FF4444',color:'#FF4444',padding:'8px 14px',cursor:'pointer',fontSize:10,fontFamily:"'Press Start 2P',monospace",outline:'none'}}>← BACK</button>
      </div>
      ...
      <div style={{display:'flex',flex:1,flexWrap:'wrap',overflow:'auto'}}>
        ...
        {/* Left */}
        <div style={{flex:'1 1 300px',padding:'24px 16px',display:'flex',flexDirection:'column',alignItems:'center',gap:16}}>
          <div style={{display:'flex',gap:8,flexWrap:'wrap',justifyContent:'center'}}>
            <div style={{background:selectedPool.darkBg,border:`2px solid ${selectedPool.color}`,color:selectedPool.color,padding:'6px 14px',fontSize:10}}>{selectedPool.icon} {selectedPool.name}</div>
            <div style={{background:'#0a2010',border:'1px solid #2a7a22',color:'#9de8b4',padding:'6px 14px',fontSize:9}}>Pool: {selectedPool.poolEth} ETH</div>
          </div>

          {phase==='waiting'&&(
            <div style={{background:'#050d05',width:'100%',border:`2px solid ${critical?'#FF4444':urgent?'#FFDD00':selectedPool.color}`,padding:'18px',textAlign:'center'}}>
          ...
          <button onClick={()=>runDrawRef.current&&runDrawRef.current()} disabled={phase==='spinning'}
            style={{padding:'12px 20px',background:phase==='spinning'?'#0a2010':'#145414',color:phase==='spinning'?'#3a7a22':'#1BF26A',border:`2px solid ${phase==='spinning'?'#3a7a22':'#1BF26A'}`,cursor:phase==='spinning'?'default':'pointer',fontSize:10,fontFamily:"'Press Start 2P',monospace",outline:'none'}}>
          ...
          <div style={{display:'flex',gap:6,flexWrap:'wrap',justifyContent:'center',width:'100%'}}>
            {PRIZE_SLOTS.map((slot,i)=>{
              const res=results[i];
              return(
                <div key={i} style={{flex:'1 1 90px',maxWidth:140,minWidth:85,background:res?`linear-gradient(160deg,${slot.color}18,#050d05)`:'#050d05',border:`2px solid ${res?slot.color:'#2a5a2a'}`,padding:'10px 6px',textAlign:'center',transition:'all .4s',boxShadow:res?.isUser?`0 0 20px ${slot.color}`:'none'}}>
        ...
        {/* Right */}
        <div style={{flex:'0 0 270px',background:'rgba(8,40,8,.8)',borderLeft:'1px solid #2a5a2a',padding:'20px',overflowY:'auto',display:'flex',flexDirection:'column',gap:14}}>
          <div>
          ...
          <div>
            ...
            {history.map(draw=>(
              <div key={draw.id} style={{background:'#0a2a0a',border:'1px solid #2a5a2a',padding:'8px 10px',marginBottom:8}}>
                ...
                {draw.winners.map((w,wi)=>(
                  <div key={wi} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'2px 0',borderTop:wi>0?'1px solid rgba(42,90,42,.4)':'none'}}>
                ...
                <a href={draw.txUrl} target="_blank" rel="noopener noreferrer"
                  style={{display:'block',marginTop:6,color:'#1BF26A',fontSize:7,textDecoration:'none',letterSpacing:1,borderTop:'1px solid rgba(27,242,106,.2)',paddingTop:5}}>
                  🔍 VERIFY TX →
                </a>
              </div>
            ))}
